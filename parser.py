...
class NotebookTaskSplitter:
    ...
    def segment_notebook(self, ipynb_path: str) -> List[Dict[str, Any]]:
        ...
        self.logger.info(f"✓ Зачитано ячеек: {len(cells)} "
        ...
        self.logger.debug("Превью py-текста с маркерами:\n" + py_text[:1000] + ("..." if len(py_text) > 1000 else ""))
...
""".strip()
    ...
    def _normalize_and_cover_segments(
        ...
        if missing:
            self.logger.warning(f"⚠ Обнаружены пропущенные ячейки: {missing}. "
